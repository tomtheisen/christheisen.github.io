...
export const inventory : InventoryItem[] = [{g:0,i:0,f:0,a:0}];
...
export function visibleItems(groupId: number){
	const group = visibleGroups().find(x => x.i === groupId);
	if(!group){return [];}
	return group.l.filter(x => x.u);
}
	
export function visibleFlavors(groupId: number, itemId: number){
